...
const map = {};
for (const fname of fs.readdirSync(__dirname, { recursive: true })) {
  if (path.extname(fname) != ".mp3") continue;
  const book = path.basename(path.dirname(fname));
  const [verse, extra] = path.basename(fname, ".mp3").split("-");
...
function genMap(map, exportName) {
  ...
  for (const [book, verses] of Object.entries(map)) {
    ...
    for (const [verse, { reg, sof }] of Object.entries(verses)) {
      ...
      out += `    ${JSON.stringify(verse.replace("_", ":"))}: () => ({${[
        mkRequire("reg", reg),
        mkRequire("sof", sof),
      ]
        .filter(Boolean)
